...
def test_should_transform_respon_to_pollution_data(requests_mock, faker):
    requests_mock.get(
...
def test_should_raise_if_error_or_no_data(requests_mock, faker):
    requests_mock.get(url=url, content=b"{}", status_code=400)
    with pytest.raises(HTTPError):
        get_pollution_data_for_point(
            faker.latitude(), faker.longitude(), faker.date_time(), 1
        )

    requests_mock.get(url=url, content=b"{}", status_code=200)
    with pytest.raises(ValueError):
        get_pollution_data_for_point(
            faker.latitude(), faker.longitude(), faker.date_time(), 1
        )
